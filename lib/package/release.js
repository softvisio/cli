...
export default class Publish {
    ...
    async run () {
        ...
        this.#majorVersionStableTag = "";

        MVST: if ( !this.#currentRelease.isPreRelease ) {
            for ( const release of status.releases ) {
                if ( this.#currentRelease.majorNumber !== release.majorNumber ) continue;

                if ( release.isPreRelease ) continue;

                if ( this.#currentRelease.lt( release ) ) break MVST;
            }

            this.#majorVersionStableTag = "v" + this.#currentRelease.majorNumber;
        }
        ...
        // set "vX" tag
        if ( this.#majorVersionStableTag ) {
            res = await repeatAction(
                async () => {
                    process.stdout.write( `Adding "${ this.#majorVersionStableTag }" tag ... ` );

                    const res = await this.#pkg.git.exec( [ "tag", "--force", "--annotate", "--message", this.#pkg.createReleaseTagAnnotation( this.#majorVersionStableTag ), this.#majorVersionStableTag ] );
        ...
        if ( this.#pkg.git.upstream ) {
            res = await repeatAction(
                async () => {
                    ...
                    const params = [ "push", "--atomic", "--force", "origin", status.head.branch, this.#currentRelease.versionString, this.#latestTag, this.#nextTag, this.#majorVersionStableTag ].filter( param => param != null && param !== "" );
    ...
    // private
    #createTagsText () {
        const tags = [ this.#latestTag, this.#nextTag, this.#majorVersionStableTag ]
            .filter( tag => tag )
