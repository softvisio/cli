...
import { compare, confirm, isEmptyObject, mergeObjects, repeatAction, shellQuote } from "#core/utils";
...
export default class Package {
    ...
    // XXX
    async updateDependencies ( { reinstall, commit } = {} ) {
    ...
    // XXX
    async getOutdatedDependencies1 ( { all, outdated, install, reinstall, commit, quiet, confirmInstall, outdatedDependencies, cache = {} } = {} ) {
        if ( !this.dependencies.hasDependencies ) return result( 200 );
        ...
        const linkedDependencies = await this.#getLinkedDependencies(),
            index = {},
            dependencies = [ ...linkedDependencies.values() ];
        ...
        for ( const name in outdatedDependencies ) {
            ...
            for ( const dependency of outdatedDependencies[ name ] ) {
                dependency.name = name;
            }

            dependencies.push( ...outdatedDependencies[ name ] );
        }

        // index dependencies
        for ( const dependency of dependencies ) {
            const id = `${ dependency.name }/${ dependency.location }/${ dependency.current }`;

            if ( index[ id ] ) {

                // use minimal wanted version for location
                // NOTE: if you comment this - add "dependency.wanted" to id
                if ( index[ id ].wanted ) {
                    if ( new SemanticVersion( dependency.wanted ?? SemanticVersion.initialVersion ).lt( index[ id ].wanted ) ) {
                        index[ id ].wanted = dependency.wanted;
                    }
                }
                else {
                    index[ id ].wanted = dependency.wanted;
                ...
                index[ id ].latest = dependency.latest;
            }
            else {
                index[ id ] = {
                    ...
                    "dependent": new Set(),
                    "topLevel": this.dependencies.has( dependency.name ) && dependency.location === path.join( this.root, "node_modules", dependency.name ),
                    "notInstalled": dependency.notInstalled,
        ...
        for ( const dependency of Object.values( index ) ) {
            ...
            // installed
                ...
                // dependency can be optional, so we additionally check dependency.current
            ...
            return fs.promises
                ...
                .then( link => {
                    ...
                    dependencies.set( name, {
                        name,
                        location,
                        "dependent": this.dependencies.names.has( name )
                            ? this.name
                            : undefined,
                        "linked": link,
                    } );
