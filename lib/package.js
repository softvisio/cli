...
import { chmodSync, pathExists, pathExistsSync } from "#core/fs";
...
export default class Package {
    ...
    async #updateDependabotConfig () {
        ...
        // github-actions
        if ( this.cliConfig?.meta.dependabot[ "github-actions" ]?.interval ) {
            const directories = new Set();

            if ( ( await glob( ".github/workflows/*.*", { "cwd": this.root } ) ).length || ( await pathExists( this.root + "/action.yaml" ) ) | ( await pathExists( this.root + "/action.yml" ) ) ) {
                directories.add( "/" );
            }
