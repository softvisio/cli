    ...
    async init () {
        ...
        if ( this.#config.externalTypes ) {
            for ( let url of this.#config.externalTypes ) {
                const [ owner, name, branch ] = url.split( "/", 3 );

                url = new URL( `https://raw.githubusercontent.com/${ owner }/${ name }/${ branch || "main" }/scli.config.yaml` );
        ...
        if ( this.#config.types ) {
            for ( const type in this.#config.types ) {
                ...
                const hash = this.#types[ type ]
                    ...
                    .toLowerCase()
                    .replaceAll( /<[^>]+>/gv, "" )
                    .replaceAll( /[!"#$%&'\(\)*+,.\/:;<=>?@\[\\\]^`\{\|\}~\u{2000}-\u{206F}\u{2E00}-\u{2E7F}]/gv, "" )
                    .replaceAll( /\s/gv, "-" )
                    .replaceAll( /-+/gv, "-" )
                    .replace( /^(\d)/v, "_$1" );
