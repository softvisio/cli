    ...
    async getReleaseTags ( { majorTagEnabled } = {} ) {
        ...
        if ( res.data ) {
            for ( const [ tag, commit ] of Object.entries( res.data ) ) {
                const isMajorTag = /^v\d+$/v.test( tag );

                // release tag
                if ( /^v\d+\.\d+\.\d+/v.test( tag ) ) {
                ...
                else if ( tag === "latest" || tag === "next" || /^v\d+\.(?:latest|next)$/v.test( tag ) || isMajorTag ) {
                    tags[ tag ] ??= {
                        isMajorTag,
                    };
