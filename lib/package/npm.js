...
export default class Npm extends NpmApi {
    ...
    async #publish ( { commitRef, accessStatus } = {} ) {
        ...
        if ( !this.pkg.isGitPackage && !this.pkg.rootPackage?.isGitPackage ) return result( [ 500, "Git package not found" ] );
        ...
        res = await this.pkg.git.exec( [ `--work-tree=${ workTree.path }`, "checkout", commitRef, "--", this.pkg.rootSlug ?? "." ] );
        if ( !res.ok ) throw res;

        const pkg = new Package( workTree.path + ( this.pkg.rootSlug
            ? "/" + this.pkg.rootSlug
            : "" ) );
